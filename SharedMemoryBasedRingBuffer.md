...
Standard PIPE and its assimilated IPCs (FIFO) have many advantages but also a few disadvantages when it comes to controlling its size and behavior or getting usage statistics.

The whole implementation can be found [Downloads/rb\_nolicense.tgz here].  It is accompanied by a stress test program which is also a usage example.

# Details #

The following ring buffer implementation has the following characteristics:

  * char granularity
  * use the one free element to solve the full/empty distinction issue
  * read and write operations are non blocking (write operation can be made blocking by compile option but this uses polling so is limited to performance testing)
  * mechanism to delete shared memory when last user closes
...
extern int rb_init(void *p_cfg);

extern int rb_open(int id, unsigned long size, unsigned long flags);

extern int rb_close(int *p_desc);

extern int rb_write(int desc, char *buf, unsigned long len);

extern int rb_read(int desc, char *buf, unsigned long len);
...
int rb_init(void *p_cfg)
{
    return 0;
}

int rb_open(int id, unsigned long size, unsigned long flags)
{
    key_t key;
    RB *p_rb;
    char *p;
...
int rb_write(int desc, char *buf, unsigned long len)
{
    RB *p_rb = (RB *)desc;
    RB_HDR *hdr ;
...
int rb_read(int desc, char *buf, unsigned long len)
{
    RB *p_rb = (RB *)desc;
    RB_HDR *hdr;
    unsigned long available;
