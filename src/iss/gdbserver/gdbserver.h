...
struct gdbserver
{
     ...
     * @param sig Signal id to send to the remote GDB.  If 0, do not send anything
     * and hold, waiting for the remote GDB to send a run command.
     ...
     * @param yesno Configure the step mode.  If 0, step mode disabled, otherwise enabled.
     */
    ...
    /** Request the ISS to ead the CPU registers and fill the buffer.
    ...
    /** Read a word from the ISS into the system without affecting the plateform timing.
    ...
    /** Write a word from the ISS into the system without affecting the plateform timing.
...
private:
    ...
    /** Read or write the plateform memory.
     * @param[in] addr Address to read / write.
     * @param[inout] buf Buffer to read to / write from.
