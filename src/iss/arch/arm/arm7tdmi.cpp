...
arm7tdmi::arm7tdmi(struct bus *bus, bool gdbserver, bool gdbstart, bool bigendian)
: mmu(bus, gdbserver, gdbstart, bigendian)
{
    // configure the ARM
    m_DeviceId = 0x41007700;
    m_IsV4 = true;
    m_IsV5 = false;
    m_IsV5e = false;
...
enum arm::fault_t
arm7tdmi::mmu_load_instr(uint32_t va, uint32_t * instr)
{
    return mmu_read_word(va, instr);
}

enum arm::fault_t
arm7tdmi::mmu_read_byte(uint32_t va, uint32_t * data)
{
    fault_t fault;
...
enum arm::fault_t
arm7tdmi::mmu_read_halfword(uint32_t va, uint32_t * data)
{
    fault_t fault;
...
enum arm::fault_t
arm7tdmi::mmu_read_word(uint32_t va, uint32_t * data)
{
    return mmu_read(va, data, ARM_WORD_TYPE);
}

enum arm::fault_t
arm7tdmi::mmu_read(uint32_t va, uint32_t * data, enum mmu::arm_data_type datatype)
{
...
enum arm::fault_t
arm7tdmi::mmu_write_byte(uint32_t va, uint32_t data)
{
    return mmu_write(va, data, ARM_BYTE_TYPE);
}

enum arm::fault_t
arm7tdmi::mmu_write_halfword(uint32_t va, uint32_t data)
{
    return mmu_write(va, data, ARM_HALFWORD_TYPE);
}

enum arm::fault_t
arm7tdmi::mmu_write_word(uint32_t va, uint32_t data)
{
    return mmu_write(va, data, ARM_WORD_TYPE);
}

enum arm::fault_t
arm7tdmi::mmu_write(uint32_t va, uint32_t data, enum mmu::arm_data_type datatype)
{
    struct tlb_entry* tlb;
    uint32_t pa;
    enum fault_t fault;
    ...
    if (MMU_CacheEnabled)
    {
        ...
        if (cache)
        {
            ...
            case ARM_HALFWORD_TYPE:
                ...
                // bit offset into the word
                offset = (((uint32_t)m_IsBigEndian * 2) ^ (va & 2)) << 3;
                cache->data[(va >> 2) & 3] = (temp & ~(0xffffL << offset)) | ((data & 0xffffL) << offset);
                break;
            ...
            case ARM_BYTE_TYPE:
                ...
                // bit offset into the word
                offset = (((uint32_t)m_IsBigEndian * 3) ^ (va & 3)) << 3;
                cache->data[(va >> 2) & 3] = (temp & ~(0xffL << offset)) | ((data & 0xffL) << offset);
                break;
...
int
arm7tdmi::mmu_mrc(uint32_t instr, uint32_t * value)
{
    ...
    case MMU_FAULT_ADDRESS:
        ...
        break;
    default:
        ARM_WARN("arm7tdmi: MMU operation not implemented reg=%d", creg);
        return -1;
        break;
    }
    *value = data;
...
int
arm7tdmi::mmu_mcr(uint32_t instr, uint32_t value)
{
    mmu_regnum_t creg = (mmu_regnum_t)(BITS(16, 19) & 0xF);
    switch (creg)
    {
...
int
arm7tdmi::gdb_read(uint64_t addr, uint8_t* dataptr, uint32_t len)
{
    ...
    {
        ARM_WARN("arm7tdmi: gdb access when cache enabled not supported yet @0x%08llX", addr);
        return 0;
    }
}

int
arm7tdmi::gdb_write(uint64_t addr, uint8_t* dataptr, uint32_t len)
{
    // if Cache disabled, do quick access
    if (MMU_Disabled || MMU_CacheDisabled)
    {
